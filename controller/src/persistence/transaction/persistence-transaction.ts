...
import { IPersistenceResult } from '../model/persistence-results.interface.js';
...
export class PersistenceTransaction {
     ...
     */
    public continue<T extends QueryResultRow = QueryResultRow>(sql: string, params: any[] = []): Promise<IPersistenceResult<T>> {
        return this.ready()
    ...
    private mapResults<T extends QueryResultRow = QueryResultRow>(input: pg.QueryResult<T>): IPersistenceResult<T> {
    ...
    private mapDataResult<T extends QueryResultRow = QueryResultRow>(input: pg.QueryResult<T>): IPersistenceResult<T> {
        return {
            inserted: input.command === 'INSERT' && input.rowCount ? input.rowCount : 0,
            updated: input.command === 'UPDATE' && input.rowCount ? input.rowCount : 0,
            deleted: input.command === 'DELETE' && input.rowCount ? input.rowCount : 0,
            results: input.command === 'SELECT' && input.rows || [],
        }
    }

    private getDefaultResult<T extends QueryResultRow = QueryResultRow>(): IPersistenceResult<T> {
        return {
            inserted: 0,
            updated: 0,
            deleted: 0,
    ...
    private addQueryResult<T extends QueryResultRow = QueryResultRow>(aggregate: IPersistenceResult<T>, item: QueryResult<T>): IPersistenceResult<T> {
