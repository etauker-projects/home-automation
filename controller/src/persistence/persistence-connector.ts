...
// import { IQueryConfig } from './model/query-config.interface.js';
...
export class PersistenceConnector {
    ...
    //  */
    // public select<T extends QueryResultRow>(sql: string, params: any[] = []): Promise<T[]> {
    //     const statements = this.splitStatements(sql);
    //     // TODO: append 'RETURNING *' to the query and return the inserted items
    ...
    // private execute<T extends QueryResultRow = QueryResultRow>(sql: string, params: any[] = [], commit: boolean): Promise<IPersistenceResult<T>> {
